...
export function ConteudoNavegacao() {
  ...
  const [itens, setItens] = useState<NavItem[]>(() =>
    [...state.navItems].sort((a, b) => a.ordem - b.ordem)
  );
  ...
  const adicionar = () => setItens((arr) => [...arr, novoItem()]);

  const salvar = () => {
    const limpos = itens
      .filter((it) => it.label.trim())
      .map((it, ordem) => ({ ...it, label: it.label.trim(), url: it.url.trim() || "#", ordem }));
    dispatch({ type: "nav/save", payload: limpos });
  };
  ...
  return (
    <>
      <Topbar
        title="Navegação"
        subtitle="Itens do menu principal do portal (header)"
        actions={
      ...
      <main className="flex-1 px-6 lg:px-10 py-8">
        <div className="max-w-[1000px] mx-auto">
            ...
            {itens.map((it, i) => (
              <div key={it.id} className="flex items-center gap-3 rounded-lg border border-border bg-background/40 p-3">
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3">
                  <input value={it.label} onChange={(e) => set(i, { label: e.target.value })} placeholder="Rótulo (ex: Governança Corporativa)" className="rounded-md border border-input bg-background px-3 py-2 text-[14px] text-foreground placeholder:text-muted-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" />
                  <input value={it.url} onChange={(e) => set(i, { url: e.target.value })} placeholder="URL (/demonstracoes ou #ancora)" className="rounded-md border border-input bg-background px-3 py-2 text-[14px] text-foreground placeholder:text-muted-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" />
                </div>
          ...
          <p className="mt-6 text-[12px] text-muted-foreground">
            URLs iniciadas por <code>/</code> são páginas internas; as iniciadas por <code>#</code> são âncoras na home.{" "}
